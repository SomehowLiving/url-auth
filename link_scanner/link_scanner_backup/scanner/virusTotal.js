...
export async function checkVirusTotal(url) {
  ...
  if (!key) {
    return { flagged: false, score: 0, positives: 0, total: 0, unavailable: true, reason: 'missing_api_key' };
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return { flagged: false, score: 0, positives: 0, total: 0, unavailable: true, reason: `http_${res.status}` };
    }
    ...
    return { flagged: false, score: 0, positives: 0, total: 0, unavailable: true, reason: 'request_failed' };
  }
}
